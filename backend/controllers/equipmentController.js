...
const getEquipmentBySerialNumber = async (req, res) => {
    if(req.user.role !== 'admin') return res.status(403).send("You are not allowed to make these changes");
...
const updateEquipmentBySerialNumber = async (req, res) => {
    if(req.user.role !== 'admin') return res.status(403).send("You are not allowed to make these changes");
    let serialNumber = req.params.serialNumber;
    const {
        name,
...
const deleteEquipmentBySerialNumber = async (req, res) => {
    if(req.user.role !== 'admin') return res.status(403).send("You are not allowed to make these changes");
    let serialNumber = req.params.serialNumber;
    try {
...
module.exports = {
  ...
  updateEquipmentBySerialNumber,
  deleteEquipmentBySerialNumber,
  getAllEquipment,
};
